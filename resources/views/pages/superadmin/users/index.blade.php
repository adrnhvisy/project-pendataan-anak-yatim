<x-app-layout>
    ...
    <div class="overflow-hidden bg-white border border-gray-100 shadow-sm rounded-xl">
        <div class="flex items-center justify-between p-5 border-b border-gray-100 bg-gray-50/50">
            ...
            <a href="{{ route('superadmin.users.create') }}" class="px-4 py-2 text-sm font-medium text-white transition-colors bg-blue-600 rounded-lg hover:bg-blue-700">
                + Tambah Akun Baru
            </a>
        </div>
                    ...
                    <tr class="transition-colors hover:bg-gray-50/50">
                        ...
                        <td class="px-6 py-4">
                            @foreach($user->roles as $role)
                                <span class="px-2.5 py-1 text-xs font-semibold text-indigo-800 bg-indigo-100 rounded-full uppercase">
                                    {{ $role->name }}
                                </span>
                            @endforeach
                        </td>
